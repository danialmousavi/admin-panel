...
const AddCart = () => {
    ...
    const handleChangeSelectedProduct = async (e, formik)=>{
      const usertoken=JSON.parse(localStorage.getItem("loginToken"))
        ...
        const res = await axios.get(`https://ecomadminapi.azhadev.ir/api/admin/products/${e}`,{
                    headers:{
            "Authorization":`Bearer ${usertoken}`
          }
        })
    ...
    const handleConfirmAddCart = async (formik)=>{
      const usertoken=JSON.parse(localStorage.getItem("loginToken"))
        ...
        if (res.status === 201) {
            Swal.fire({
                title: res.data.message||"سبد خرید با موفقیت اضافه شد",
                icon: "success",
                                    ...
                                    <Form>
                                        <div className="row my-3 justify-content-center">
                                            ...
                                            <div className="col-12 col-md-4 col-lg-3 my-1">
                                                ...
                                                <br/>
                                                <ErrorMessage name='product_id' >{ msg => <div style={{ color: 'red' }}>{msg}</div> }</ErrorMessage>
                                            </div>
